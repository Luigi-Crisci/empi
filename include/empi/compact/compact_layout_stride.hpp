...
template<order DataOrdering = row_major, typename T, template<typename, size_t...> typename Extents, typename Layout,
    typename Accessor, typename IdxType, size_t... Idx>
constexpr auto layout_stride_compact_impl2_d(
    const Kokkos::mdspan<T, Extents<IdxType, Idx...>, Layout, Accessor> &view) {
    static_assert(Extents<IdxType, Idx...>::rank() == 2, "Only 2D mdspan are supported");
    ...
    if(unit_stride.is_contiguous) { return contiguous_layout_compact_impl(view); }
    ...
    if(!has_unit_stride) { return non_contiguous_layout_compact_impl(std::move(view)); }
