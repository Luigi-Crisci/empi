...
namespace layouts{
	...
	struct column_layout {
		...
		static auto build(std::ranges::forward_range auto&& view, Extents<K,Idx...> extents, const int col){
		...
		static std::unique_ptr<T[]>&& compact(const column_view<T,Extents>& view){
			stdex::mdspan<int, stdex::dextents<size_t, 1>,stdex::layout_stride> x;
	...
	struct contiguous_layout{
		...
		static auto build(std::ranges::forward_range auto&& view){
			...
			using T = std::ranges::range_value_t<decltype(view)>;
			return stdex::mdspan<T, extent_type, std::experimental::layout_right>(
