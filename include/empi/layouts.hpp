...
struct block_layout {
  ...
  template <template <typename, size_t...> typename Extents, typename T,
            typename Size, typename Stride, size_t... idx>
  [[nodiscard]] static constexpr auto
  build(std::ranges::forward_range auto &&view, Extents<T, idx...> extents,
        Size &&blocks, Stride &&strides) {
    // TODO: Check sizes against view
    // TODO: Tiled layout should work on every dimension
    using view_data_type = std::ranges::range_value_t<decltype(view)>;
  ...
  struct tiled_block_layout {
    template <typename Extents> struct mapping {
      ...
      constexpr mapping(const Extents &ext, std::size_t size,
                        std::size_t stride)
  ...
  struct block_block_layout {
    ...
    template <typename Extents> struct mapping {
      ...
      constexpr size_type operator()(index idx) const noexcept {
        ...
        return idx + (num_blocks + remainder) * _offsets[0] +
               num_blocks * _offsets[1];
      }
  ...
  struct bucket_block_layout {
    ...
    template <typename Extents> struct mapping {
      ...
      constexpr mapping(const Extents &ext, const std::span<size_t, 2> &sizes,
                        size_t stride)
  ...
  struct alternating_block_layout {
    ...
    template <typename Extents> struct mapping {
      ...
      constexpr mapping(const Extents &ext, const std::span<size_t, 2> &sizes,
                        const std::span<size_t, 2> &strides)
          : _extents(ext), _sizes(sizes), _strides(strides), _sum_blocks(0),
            _sum_diffs(0) {
        // At least one block and stride
  ...
  compact(const stdex::mdspan<T, Extents<idx_type, idx...>, Layout, Accessor>
    ...
    auto compact_f = [&](std::ranges::forward_range auto &&sizes) {
      ...
      for (int pos = 0, block = 0; pos < view.extent(0);
           pos += sizes[block], block = (block + 1) % num_blocks) {
    ...
    if constexpr (std::is_same_v<Layout, bucket_block_layout> ||
                  std::is_same_v<Layout, alternating_block_layout>) {
