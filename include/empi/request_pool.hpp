...
class request_pool {
  public:
    explicit request_pool(size_t size) : data(size), base_size(size), window(default_windows_size) {
        for(auto &req : data) req = std::make_shared<async_event>();
        head = 0;
        tail = size - 1;
    }
    ...
    void waitall() {
        ...
        if(empi::details::abs(head, tail) <= 1) return;
        ...
        while(tail != head) {
            ...
            if(err == MPI_ERR_REQUEST)
                throw std::runtime_error(
                    "Found an invalid request while compacting the request_pool. This should never happen");
            if(!flag) break;
            tail = (tail + 1) % data.size();
            mov++;
        }
