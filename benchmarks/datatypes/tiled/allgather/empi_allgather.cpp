...
int main(int argc, char **argv) {
    ...
    auto run_bench = [&](auto data_t_v) {
        ...
        t_view1 = MPI_Wtime();
        auto view = empi::layouts::block_layout::build(myarr, Kokkos::dextents<size_t, 1>(view_size), A, B);
        t_view2 = MPI_Wtime();

        message_group->run([&](empi::MessageGroupHandler<type> &mgh) {
            // Warmup
            mgh.barrier();
            for(auto iter = 0; iter < WARMUP; iter++) mgh.Allgather(view, view_size, recv, view_size);
            mgh.barrier();
            ...
            for(auto iter = 0; iter < max_iter; iter++) { mgh.Allgather(ptr.get(), view_size, recv, view_size); }
