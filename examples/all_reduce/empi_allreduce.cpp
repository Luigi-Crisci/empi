...
int main(int argc, char **argv) {
    ...
    message_group->run([&](empi::MessageGroupHandler<value_type, empi::Tag{0}, empi::NOSIZE> &mgh) {
        // Warmup
        mgh.barrier();
        ...
        for(auto iter = 0; iter < max_iter; iter++) { mgh.Allreduce(myarr.data(), dest.data(), n, MPI_SUM); }
